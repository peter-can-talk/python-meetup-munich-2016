...
\begin{itemize}
  ...
  \pause\item $\textbf{x}$ contains features, such as $(\text{lunch, dinner})$ % predicting how happy a person is based on what they had for lunch and dinner (very good metric)
...
\begin{slide}{The Task, $T$}
  ...
  \begin{itemize}
    ...
    \pause\item Predictive Policing %Placing crime suspects into different threat levels based on crime history
  \end{itemize}
\end{slide}
...
\begin{slide}{The Task, $T$}
  ...
  \begin{itemize}
    ...
    \pause\item Algorithmic Trading % Predict future prices given the current and past state of the market. Generally anything that has to do with money.
    \pause\item Predicting the market price of a house % Given its features of size, age, pool
...
\begin{slide}{The Task, $T$}
  ...
  \begin{itemize}
    ...
    \pause\item Similar to \emph{Translation} % Transforming a sequence of symbols from one structured representation to another, such as English
  \end{itemize}
\end{slide}
...
\begin{slide}{Experience $E$}
  ...
  \vspace{1.3cm}
  \large
  \pause Unsupervised Learning % k-means clustering, synthesis tasks, no instructor, the algorithm must make sense of the data itself
  \hspace{1cm}
  \pause Supervised Learning % Most of deep learning, e.g. image classification; there is an instructor guiding the algorithm, labeling the input. The task of the algorithm is to understand what inputs produce what outputs.
\end{slide}
